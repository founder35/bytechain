...
CHAINID="${CHAIN_ID:-bytechain_9000-1}"
BASE_DENOM="aevmos"
MONIKER="bytechain-testnet"
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	bytechaind config set client chain-id "$CHAINID" --home "$HOMEDIR"
	bytechaind config set client keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	echo "$VAL_MNEMONIC" | bytechaind keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER1_MNEMONIC" | bytechaind keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER2_MNEMONIC" | bytechaind keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER3_MNEMONIC" | bytechaind keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER4_MNEMONIC" | bytechaind keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"

	# Set moniker and chain-id for Evmos (Moniker can be anything, chain-id must be an integer)
	bytechaind init $MONIKER -o --chain-id "$CHAINID" --home "$HOMEDIR"
	...
	bytechaind add-genesis-account "$(bytechaind keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 100000000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	bytechaind add-genesis-account "$(bytechaind keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	bytechaind add-genesis-account "$(bytechaind keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	bytechaind add-genesis-account "$(bytechaind keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"
	bytechaind add-genesis-account "$(bytechaind keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000$BASE_DENOM --keyring-backend "$KEYRING" --home "$HOMEDIR"

	# Sign genesis transaction
	bytechaind gentx "$VAL_KEY" 1000000000000000000000$BASE_DENOM --gas-prices ${BASEFEE}$BASE_DENOM --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
	## In case you want to create multiple validators at genesis
	## 1. Back to `bytechaind keys add` step, init more keys
	## 2. Back to `bytechaind add-genesis-account` step, add balance for those
	## 3. Clone this ~/.bytechaind home directory into some others, let's say `~/.clonedEvmosd`
	## 4. Run `gentx` in each of those folders
	## 5. Copy the `gentx-*` folders under `~/.clonedEvmosd/config/gentx/` folders into the original `~/.bytechaind/config/gentx`

	# Collect genesis tx
	bytechaind collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	bytechaind validate-genesis --home "$HOMEDIR"
